...
use crate::models::{
    ...
    VaultEntries, VaultStatus, VaultxExportRequest, VaultxExportResult, VaultxImportRequest,
...
static STATE: once_cell::sync::Lazy<Mutex<VaultState>> =
    once_cell::sync::Lazy::new(|| Mutex::new(VaultState::default()));
...
fn set_state(master_key: [u8; 32], device_key: [u8; 32], meta: &MetaFile) {
    let mut guard = STATE.lock().unwrap_or_else(|e| {
        eprintln!("[codebook] state poisoned, recovering");
        e.into_inner()
    });
...
fn require_state() -> Result<([u8; 32], [u8; 32], String), String> {
    let guard = STATE.lock().map_err(|e| {
        drop(e.into_inner());
        "state lock error".to_string()
    })?;
    let dk = guard
        .device_key
        .ok_or_else(|| "当前为锁定状态".to_string())?;
    let mk = guard.master_key.ok_or_else(|| "主密钥不存在".to_string())?;
    let device_id = guard
        .device_id
        .clone()
...
pub fn list_entries(app: AppHandle) -> Result<VaultEntries, String> {
    ...
    Ok(VaultEntries {
        ...
        deleted,
        config: index.config,
        global_version: STATE.lock().ok().map(|s| s.global_version).unwrap_or(0),
    })
}
...
fn save_entry_batch(
    ...
    if !account.account_identity.is_empty()
        && !index
            .config
            .user_identities
            .contains(&account.account_identity)
    {
        index
            .config
            .user_identities
            .push(account.account_identity.clone());
    }
...
pub fn delete_entry(app: AppHandle, id: String) -> Result<VaultEntries, String> {
    ...
    if path.exists() {
        let blob = read_encrypted_blob(&path)?;
        let mut acc: SecureAccount = serde_json::from_slice(&decrypt_with_device(
            &dk,
            &derive_entry_context(&id),
            &blob,
        )?)
...
pub fn export_vaultx(
    app: AppHandle,
    req: VaultxExportRequest,
) -> Result<VaultxExportResult, String> {
    ...
    let mut sig_hmac =
        <HmacSha256 as KeyInit>::new_from_slice(&sig_mac).map_err(|e| e.to_string())?;
    ...
    zip.start_file("vaultx/vaultx.dat", options)
        .map_err(|e| e.to_string())?;
    zip.write_all(&dat_bytes).map_err(|e| e.to_string())?;

    zip.start_file("vaultx/vaultx.attach", options)
        .map_err(|e| e.to_string())?;
    zip.write_all(&attach_bytes).map_err(|e| e.to_string())?;

    zip.start_file("vaultx/vaultx.meta", options)
        .map_err(|e| e.to_string())?;
    zip.write_all(&meta_bytes).map_err(|e| e.to_string())?;

    zip.start_file("vaultx/vaultx.sig", options)
        .map_err(|e| e.to_string())?;
...
pub fn import_vaultx(app: AppHandle, req: VaultxImportRequest) -> Result<VaultEntries, String> {
    ...
    let attachments: HashMap<String, VaultxV2Attachment> = if !attach_bytes.is_empty() {
        let attach_blob: EncryptedBlob =
            serde_json::from_slice(&attach_bytes).map_err(|e| e.to_string())?;
        let (attach_enc, attach_mac) = derive_file_keys(&mk, "vaultx-attach")?;
...
pub fn preview_vaultx(
    app: AppHandle,
    req: VaultxPreviewRequest,
) -> Result<VaultxPreviewResult, String> {
    let root = vault_root(&app);
    let (dk, mk, device_id) = require_state()?;
    if is_current_device_revoked(&root, &dk, &device_id)? {
    ...
    for import_acc in &import_active {
        let local_match = local_active
            .iter()
            .find(|a| a.name == import_acc.title && a.account_identity == import_acc.username);
        match local_match {
            Some(local_acc) => {
    ...
    for local_acc in &local_active {
        let in_import = import_active
            .iter()
            .any(|a| a.title == local_acc.name && a.username == local_acc.account_identity);
        if !in_import {
            diffs.push(AccountDiff {
