...
pub fn convert(app: AppHandle, req: ConvertRequest) -> Result<ConvertResult, String> {
  ...
  let output_dir = if is_single {
    output_path.parent().map(|p| p.to_path_buf()).unwrap_or(base_dir.clone())
  } else {
    output_path.clone()
  };
  ...
  if is_single {
    ...
    for (idx, path) in files.iter().enumerate() {
      emit_progress(&app, "convert", idx + 1, total, path.to_string_lossy().as_ref(), id);
  ...
  Ok(ConvertResult {
    batches: if is_single { 1 } else { (files.len() + batch_size.max(1) - 1) / batch_size.max(1) },
...
fn add_page_from_image(doc: &mut Document, path: &Path, img_idx: usize) -> Result<Option<ObjectId>, String> {
