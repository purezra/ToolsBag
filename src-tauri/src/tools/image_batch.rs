...
use image::imageops;
use image::{
    image_dimensions, ColorType, ExtendedColorType, GenericImageView, ImageBuffer, ImageDecoder,
    ImageFormat, ImageReader, Luma, LumaA, Rgba, RgbaImage,
};
...
type TiffPageData = (Vec<u8>, Option<Vec<u8>>, &'static str);
...
pub fn convert(app: AppHandle, req: Convert3Request) -> Result<Convert3Result, String> {
    ...
    Ok(Convert3Result {
        batches: if is_single {
            ...
            paths.len().div_ceil(batch_size.max(1))
        },
    ...
    if let Some(f) = format {
        ...
        if !fmt.is_empty() && !ext.is_empty() && fmt != ext {
            note_warning(
                warnings,
                path,
                &format!("扩展名与实际格式不符: 扩展名 {} / 实际 {}", ext, fmt),
            );
        }
    }
    ...
    if is_jpeg && !jpeg_rgb && jpeg_is_cmyk(path) {
        ...
        if let Ok(file) = std::fs::File::open(path) {
            ...
            if let Ok(exif) = exif::Reader::new().read_from_container(&mut reader) {
                ...
                {
                    rotated_90_or_270 = matches!(orientation, 5..=8);
                    if orientation != 1 {
                        note_warning(
                            warnings,
                            path,
                            &format!("应用 EXIF 旋转: {} (CMYK)", orientation),
                        );
                    }
                }
            }
        ...
        match dyn_img.color() {
            ...
            _ => {
                ...
                match dyn_img.color() {
                    ...
                    other => {
                        note_warning(
                            warnings,
                            path,
                            &format!("非 RGB 颜色空间 {:?}，已转换为 RGB", other),
                        );
                    }
                }
                let (rgb_data, smask) = if dyn_img.color().has_alpha() {
                    ...
                    (
                        flatten_rgba_to_rgb(&rgba).into_raw(),
                        Some(extract_alpha(&rgba)),
                    )
                } else {
...
fn prepare_png_optimized(
    path: &Path,
    _warnings: &Arc<Mutex<Vec<String>>>,
) -> Result<Vec<PageBuf>, ImageIssue> {
...
fn convert_tiff_data_u8(
    ...
    warnings: &Arc<Mutex<Vec<String>>>,
    path: &Path,
) -> Result<TiffPageData, ImageIssue> {
...
pub fn convert_to_epub(
    ...
    for (_idx, image_id, xhtml_id, image_filename, xhtml_filename, jpeg_data, xhtml_content) in
        successful_results
    {
        // 写入图片
        zip.start_file(format!("OEBPS/{}", image_filename), options_deflate)
            .map_err(|e| e.to_string())?;
        zip.write_all(&jpeg_data).map_err(|e| e.to_string())?;

        // 写入 XHTML 页面
        zip.start_file(format!("OEBPS/{}", xhtml_filename), options_deflate)
            .map_err(|e| e.to_string())?;
    ...
    // 写入 CSS
    zip.start_file("OEBPS/styles/style.css", options_deflate)
        .map_err(|e| e.to_string())?;
...
fn process_image_for_epub(path: &Path, page_size: EpubPageSize) -> Result<Vec<u8>, ImageIssue> {
    ...
    let (target_w, target_h) = if is_portrait {
        // 竖屏图片：目标尺寸保持竖屏 (窄高)
        (
            page_size.height.min(page_size.width),
            page_size.height.max(page_size.width),
        )
    } else {
        // 横屏图片：目标尺寸保持横屏 (宽矮)
        (
