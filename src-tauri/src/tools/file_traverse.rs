use crate::models::{
    DirectoryPreviewFile, DirectoryPreviewResult, FormatStat, MediaFileInfo, MediaPreviewFile,
...
pub fn traverse_and_copy(app: AppHandle, req: TraverseRequest) -> Result<TraverseResult, String> {
    ...
    let tasks: Vec<(usize, PathBuf, String, String, String, u64)> = files
        ...
        .map(|(idx, (path, ext, prefix, size))| {
            let original_name = path.file_stem().and_then(|s| s.to_str()).unwrap_or("file");
    ...
    Ok(TraverseResult {
        ...
        throughput_bytes,
        format_stats,
        problems: problems.lock().map(|g| g.clone()).unwrap_or_default(),
        log_path,
...
pub fn preview_directory_files(input: PathBuf) -> Result<DirectoryPreviewResult, String> {
    if !input.exists() {
        return Err("输入目录不存在".into());
    }
    if !input.is_dir() {
    ...
    {
        if files.len() >= 50_000 {
            return Err("预览文件数超过上限（50000），请缩小范围后重试".into());
        }
        let path = entry.path().to_path_buf();
        let size = fs::metadata(&path).map(|meta| meta.len()).unwrap_or(0);
        total_size = total_size.saturating_add(size);
        files.push(DirectoryPreviewFile {
            name: path
                .file_name()
...
#[cfg(test)]
mod tests {
    use super::preview_directory_files;
    use std::fs;
    use uuid::Uuid;

    #[test]
    fn preview_directory_files_counts_nested_files() {
        let root = std::env::temp_dir().join(format!("toolsbag-preview-{}", Uuid::new_v4()));
        let nested = root.join("nested");
        fs::create_dir_all(&nested).expect("create nested temp dir");
        fs::write(root.join("a.txt"), b"abc").expect("write a");
        fs::write(nested.join("b.txt"), b"12345").expect("write b");

        let result = preview_directory_files(root.clone()).expect("preview");
