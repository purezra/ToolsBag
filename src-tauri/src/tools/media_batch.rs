...
fn probe_video(path: &Path) -> (Option<f64>, Option<u32>, Option<u32>, Option<f64>, Option<String>, Option<String>, Option<String>) {
  // 优先使用 MediaInfo
...
fn probe_video_ffprobe(path: &Path) -> (Option<f64>, Option<u32>, Option<u32>, Option<f64>, Option<String>, Option<String>, Option<String>) {
  let mut cmd = Command::new("ffprobe");
  cmd.args([
    "-v",
    "error",
  ...
  let output = match output {
    Ok(o) => o,
    Err(e) => return (None, None, None, None, None, None, Some(format!("ffprobe 不可用: {}", e))),
  };
  ...
  let parsed: Ffprobe = serde_json::from_slice(&output.stdout)
    ...
    .unwrap_or(Ffprobe { streams: None, format: None });
...
fn probe_image(path: &Path) -> (Option<u32>, Option<u32>, Option<String>, Option<String>, Option<String>, Option<String>) {
  let mut cmd = Command::new("exiftool");
  cmd.args([
    "-json",
    "-n",
  ...
  match cmd.output() {
    Ok(output) if output.status.success() => {
      ...
      if let Some(first) = parsed.first() {
        let width = first.get("ImageWidth").and_then(|v| v.as_u64()).map(|v| v as u32);
        let height = first.get("ImageHeight").and_then(|v| v.as_u64()).map(|v| v as u32);
        let make = first.get("Make").and_then(|v| v.as_str()).unwrap_or("").trim();
        let model = first.get("Model").and_then(|v| v.as_str()).unwrap_or("").trim();
    ...
    Ok(output) => {
      ...
      return (w, h, device, taken_at, focal, inner_err.or(Some(fallback_err)));
    }
    Err(e) => {
      let (w, h, device, taken_at, focal, inner_err) = probe_image_fallback(path);
...
fn probe_image_fallback(path: &Path) -> (Option<u32>, Option<u32>, Option<String>, Option<String>, Option<String>, Option<String>) {
  ...
  let (width, height) = match dims {
    ...
    Err(e) => return (None, None, None, None, None, Some(format!("读取尺寸失败: {}", e))),
  };
